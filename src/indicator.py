...
def get_return_by_period(df_close, periods=12):
...
def is_strong(df_close, window=12):
    ...
    df_roc = get_return_by_period(df_close)
...
def rsi(df):
    ...
    # loss, multiple -1 to positive values
    df_loss = -1 * df_change.where(df_change < 0)
    ...
    df_avg_gain.iloc[periods] = df_gain[1:periods+1].mean()
    # First Average Loss = Sum of Losses over the past 14 periods / 14
    df_avg_loss.iloc[periods] = df_loss[1:periods+1].mean()

    for index in range(periods+1, len(df)):
        # Average Gain = [(previous Average Gain) x 13 + current Gain] / 14.
        df_avg_gain.iloc[index] = (
            df_avg_gain.iloc[index-1] * 13 + df_gain.iloc[index])/periods

        # Average Loss = [(previous Average Loss) x 13 + current Loss] / 14.
        df_avg_loss.iloc[index] = (
            df_avg_loss.iloc[index-1] * 13 + df_loss.iloc[index])/periods
...
def ATR(df):
    ...
    # First ATR = Sum of TR over the past 14 periods / 14
    df_ATR.iloc[periods-1] = df_TR[0:periods].mean()
    ...
    for index in range(periods, len(df)):
        ...
        df_ATR.iloc[index] = (df_ATR.iloc[index-1] *
                              13 + df_TR.iloc[index])/periods
...
def getBeta(stock_df, index_df):
    ...
    rs = get_return_by_period(stock_df.Close, periods=1)
    rb = get_return_by_period(index_df.Close, periods=1)
...
def percent_KD(df, periods=14):
    ...
    for index in range(0, finish):
        ...
        K_.iloc[position] = (current_close - Lowest_Low) / \
...
def OBV(df_volume, df_close):
    ...
    for symbol in df_volume.columns:
        for index in range(1, len(df_volume)):
            ...
            df_OBV.iloc[index][symbol] = df_OBV.iloc[index -
                                                     1][symbol] + current_volume
...
def get_change_price(df_open, df_close):
    ...
    df_result = pd.concat(
        [method1, method2, method3, method4, method5], axis=1, join='inner')

    df_result.columns = ['<CLOSE_OPEN>', '<OPEN_POPEN>',
                         '<OPEN_PCLOSE>', '<CLOSE_POPEN>', '<CLOSE_PCLOSE>']
    return df_result
...
def listLowVolatility(df):
    ...
    # index of min values is at first of the queue
    indexList = np.argsort(countVolatility)
